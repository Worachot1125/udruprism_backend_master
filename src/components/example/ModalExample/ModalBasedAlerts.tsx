"use client";
import React from "react";
import ComponentCard from "../../common/ComponentCard";
import { Modal } from "../../ui/modal";
import { useModal } from "@/hooks/useModal";

/* ========= Reusable SVGs ========= */
const Blob: React.FC<{ className: string }> = ({ className }) => (
  <svg
    className={className}
    width="90"
    height="90"
    viewBox="0 0 90 90"
    aria-hidden="true"
  >
    <path d="M34.364 6.85053C38.6205 -2.28351 51.3795 -2.28351 55.636 6.85053C58.0129 11.951 63.5594 14.6722 68.9556 13.3853C78.6192 11.0807 86.5743 21.2433 82.2185 30.3287C79.7862 35.402 81.1561 41.5165 85.5082 45.0122C93.3019 51.2725 90.4628 63.9451 80.7747 66.1403C75.3648 67.3661 71.5265 72.2695 71.5572 77.9156C71.6123 88.0265 60.1169 93.6664 52.3918 87.3184C48.0781 83.7737 41.9219 83.7737 37.6082 87.3184C29.8831 93.6664 18.3877 88.0266 18.4428 77.9156C18.4735 72.2695 14.6352 67.3661 9.22531 66.1403C-0.462787 63.9451 -3.30193 51.2725 4.49185 45.0122C8.84391 41.5165 10.2138 35.402 7.78151 30.3287C3.42572 21.2433 11.3808 11.0807 21.0444 13.3853C26.4406 14.6722 31.9871 11.951 34.364 6.85053Z" />
  </svg>
);

// ทำให้ Glyph* รับ className แล้วเอาไปใส่ที่ <svg> โดยตรง
const GlyphSuccess: React.FC<{ className?: string }> = ({ className }) => (
  <svg width="38" height="38" viewBox="0 0 38 38" aria-hidden="true" className={className}>
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M5.9375 19.0004C5.9375 11.7854 11.7864 5.93652 19.0014 5.93652C26.2164 5.93652 32.0653 11.7854 32.0653 19.0004C32.0653 26.2154 26.2164 32.0643 19.0014 32.0643C11.7864 32.0643 5.9375 26.2154 5.9375 19.0004ZM24.7855 17.0575C25.3713 16.4717 25.3713 15.522 24.7855 14.9362C24.1997 14.3504 23.25 14.3504 22.6642 14.9362L17.7177 19.8827L15.3387 17.5037C14.7529 16.9179 13.8031 16.9179 13.2173 17.5037C12.6316 18.0894 12.6316 19.0392 13.2173 19.625L16.657 23.0647C16.9383 23.346 17.3199 23.504 17.7177 23.504C18.1155 23.504 18.4971 23.346 18.7784 23.0647L24.7855 17.0575Z"
    />
  </svg>
);

const GlyphInfo: React.FC<{ className?: string }> = ({ className }) => (
  <svg width="38" height="38" viewBox="0 0 38 38" aria-hidden="true" className={className}>
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M5.85547 18.9998C5.85547 11.7396 11.7411 5.854 19.0013 5.854C26.2615 5.854 32.1471 11.7396 32.1471 18.9998C32.1471 26.2601 26.2615 32.1457 19.0013 32.1457C11.7411 32.1457 5.85547 26.2601 5.85547 18.9998ZM16.9999 11.9145C16.9999 13.0191 17.8953 13.9145 18.9999 13.9145H19.0015C20.106 13.9145 21.0015 13.0191 21.0015 11.9145C21.0015 10.81 20.106 9.91454 19.0015 9.91454H18.9999C17.8953 9.91454 16.9999 10.81 16.9999 11.9145ZM19.0014 27.8171C18.173 27.8171 17.5014 27.1455 17.5014 26.3171V17.3293C17.5014 16.5008 18.173 15.8293 19.0014 15.8293C19.8299 15.8293 20.5014 16.5008 20.5014 17.3293L20.5014 26.3171C20.5014 27.1455 19.8299 27.8171 19.0014 27.8171Z"
    />
  </svg>
);

const GlyphWarning: React.FC<{ className?: string }> = ({ className }) => (
  <svg width="38" height="38" viewBox="0 0 38 38" aria-hidden="true" className={className}>
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M32.1445 19.0002C32.1445 26.2604 26.2589 32.146 18.9987 32.146C11.7385 32.146 5.85287 26.2604 5.85287 19.0002C5.85287 11.7399 11.7385 5.85433 18.9987 5.85433C26.2589 5.85433 32.1445 11.7399 32.1445 19.0002ZM21.0001 26.0855C21.0001 24.9809 20.1047 24.0855 19.0001 24.0855H18.9985C17.894 24.0855 16.9985 24.9809 16.9985 26.0855C16.9985 27.19 17.894 28.0855 18.9985 28.0855H19.0001C20.1047 28.0855 21.0001 27.19 21.0001 26.0855ZM18.9986 10.1829C19.827 10.1829 20.4986 10.8545 20.4986 11.6829V20.6707C20.4986 21.4992 19.827 22.1707 18.9986 22.1707C18.1701 22.1707 17.4986 21.4992 17.4986 20.6707V11.6829C17.4986 10.8545 18.1701 10.1829 18.9986 10.1829Z"
    />
  </svg>
);

const GlyphError: React.FC<{ className?: string }> = ({ className }) => (
  <svg width="38" height="38" viewBox="0 0 38 38" aria-hidden="true" className={className}>
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M9.62684 11.7496C9.04105 11.1638 9.04105 10.2141 9.62684 9.6283C10.2126 9.04252 11.1624 9.04252 11.7482 9.6283L18.9985 16.8786L26.2485 9.62851C26.8343 9.04273 27.7841 9.04273 28.3699 9.62851C28.9556 10.2143 28.9556 11.164 28.3699 11.7498L21.1198 18.9999L28.3699 26.25C28.9556 26.8358 28.9556 27.7855 28.3699 28.3713C27.7841 28.9571 26.8343 28.9571 26.2485 28.3713L18.9985 21.1212L11.7482 28.3715C11.1624 28.9573 10.2126 28.9573 9.62684 28.3715C9.04105 27.7857 9.04105 26.836 9.62684 26.2502L16.8771 18.9999L9.62684 11.7496Z"
    />
  </svg>
);

/* ========= Generic alert content ========= */
type Variant = "success" | "info" | "warning" | "error";

const styles: Record<
  Variant,
  { blob: string; glyph: string; title: string; button: string }
> = {
  success: {
    blob: "fill-success-50 dark:fill-success-500/15",
    glyph: "fill-success-600 dark:fill-success-500",
    title: "Well Done!",
    button: "bg-success-500 hover:bg-success-600 text-white",
  },
  info: {
    blob: "fill-blue-light-50 dark:fill-blue-light-500/15",
    glyph: "fill-blue-light-500",
    title: "Information Alert!",
    button: "bg-blue-light-500 hover:bg-blue-light-600 text-white",
  },
  warning: {
    blob: "fill-warning-50 dark:fill-warning-500/15",
    glyph: "fill-warning-600 dark:fill-orange-400",
    title: "Warning Alert!",
    button: "bg-warning-500 hover:bg-warning-600 text-white",
  },
  error: {
    blob: "fill-error-50 dark:fill-error-500/15",
    glyph: "fill-error-600 dark:fill-error-500",
    title: "Danger Alert!",
    button: "bg-error-500 hover:bg-error-600 text-white",
  },
};

const GlyphByVariant: Record<Variant, React.FC<{ className?: string }>> = {
  success: GlyphSuccess,
  info: GlyphInfo,
  warning: GlyphWarning,
  error: GlyphError,
};

function AlertContent({
  variant,
  title,
  body,
  actionText = "Okay, Got It",
  onAction,
  titleId,
  descId,
}: {
  variant: Variant;
  title?: string;
  body: string;
  actionText?: string;
  onAction?: () => void;
  titleId: string;
  descId: string;
}) {
  const S = styles[variant];
  const Glyph = GlyphByVariant[variant];
  return (
    <div className="text-center" aria-labelledby={titleId} aria-describedby={descId}>
      <div className="relative z-10 mb-7 flex items-center justify-center">
        <Blob className={S.blob} />
        {/* ใส่ class สีลงบน <svg> ของ Glyph โดยตรง */}
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
          <Glyph className={S.glyph} />
        </div>
      </div>

      <h4 id={titleId} className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90 sm:text-title-sm">
        {title ?? S.title}
      </h4>

      <p id={descId} className="text-sm leading-6 text-gray-500 dark:text-gray-400">
        {body}
      </p>

      <div className="mt-7 flex w-full items-center justify-center gap-3">
        <button
          type="button"
          onClick={onAction}
          className={`shadow-theme-xs sm:w-auto w-full rounded-lg px-4 py-3 text-sm font-medium ${S.button}`}
        >
          {actionText}
        </button>
      </div>
    </div>
  );
}

/* ========= Page ========= */
export default function ModalBasedAlerts() {
  const successModal = useModal();
  const infoModal = useModal();
  const warningModal = useModal();
  const errorModal = useModal();

  return (
    <ComponentCard title="Modal Based Alerts">
      <div className="flex flex-wrap items-center gap-3">
        <button
          type="button"
          onClick={successModal.openModal}
          className="rounded-lg bg-success-500 px-4 py-3 text-sm font-medium text-white shadow-theme-xs hover:bg-success-600"
        >
          Success Alert
        </button>
        <button
          type="button"
          onClick={infoModal.openModal}
          className="rounded-lg bg-blue-light-500 px-4 py-3 text-sm font-medium text-white shadow-theme-xs hover:bg-blue-light-600"
        >
          Info Alert
        </button>
        <button
          type="button"
          onClick={warningModal.openModal}
          className="rounded-lg bg-warning-500 px-4 py-3 text-sm font-medium text-white shadow-theme-xs hover:bg-warning-600"
        >
          Warning Alert
        </button>
        <button
          type="button"
          onClick={errorModal.openModal}
          className="rounded-lg bg-error-500 px-4 py-3 text-sm font-medium text-white shadow-theme-xs hover:bg-error-600"
        >
          Danger Alert
        </button>
      </div>

      {/* Success */}
      <Modal
        isOpen={successModal.isOpen}
        onClose={successModal.closeModal}
        className="max-w-[600px] p-5 lg:p-10"
      >
        <AlertContent
          variant="success"
          body="Lorem ipsum dolor sit amet consectetur. Feugiat ipsum libero tempor felis risus nisi non. Quisque eu ut tempor curabitur."
          onAction={successModal.closeModal}
          titleId="success-title"
          descId="success-desc"
        />
      </Modal>

      {/* Info */}
      <Modal
        isOpen={infoModal.isOpen}
        onClose={infoModal.closeModal}
        className="max-w-[600px] p-5 lg:p-10"
      >
        <AlertContent
          variant="info"
          body="Lorem ipsum dolor sit amet consectetur. Feugiat ipsum libero tempor felis risus nisi non. Quisque eu ut tempor curabitur."
          onAction={infoModal.closeModal}
          titleId="info-title"
          descId="info-desc"
        />
      </Modal>

      {/* Warning */}
      <Modal
        isOpen={warningModal.isOpen}
        onClose={warningModal.closeModal}
        className="max-w-[600px] p-5 lg:p-10"
      >
        <AlertContent
          variant="warning"
          body="Lorem ipsum dolor sit amet consectetur. Feugiat ipsum libero tempor felis risus nisi non. Quisque eu ut tempor curabitur."
          onAction={warningModal.closeModal}
          titleId="warn-title"
          descId="warn-desc"
        />
      </Modal>

      {/* Error */}
      <Modal
        isOpen={errorModal.isOpen}
        onClose={errorModal.closeModal}
        className="max-w-[600px] p-5 lg:p-10"
      >
        <AlertContent
          variant="error"
          body="Lorem ipsum dolor sit amet consectetur. Feugiat ipsum libero tempor felis risus nisi non. Quisque eu ut tempor curabitur."
          onAction={errorModal.closeModal}
          titleId="error-title"
          descId="error-desc"
        />
      </Modal>
    </ComponentCard>
  );
}
